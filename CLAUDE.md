...
12. 多前缀循环 → 配 A(1)/B(2)/C(2) 开循环连发:console `style prefix cycle: 使用「名称」(N/M)` 与复制到的 finalPrompt 前缀应按 A,B,B,C,C,A 轮换;刷新页面后接着上次进度走;关循环恒用 ⊙ 选中行;改小 count/删行后下一次取值按快进/取模自愈;复制配置档新档从第一条走、两档列表互不影响;旧单值配置档刷新后自动迁移为列表第一项(全档一次性);每行名称输入框改完落盘,刷新后保留(存量条目名称为空正常回退)
