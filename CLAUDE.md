...
- `applyStylePrefixToPrompt` — 正向风格前缀拼接(`generateViaComfyInner` 内唯一拼装点,自动/手动/重生成/测试/重试全路径共用):`[%style%]` 占位符原位替换(split/join 字面全量替换,无 `$` 展开语义)/ `[Shot 1]` 后插入(前缀剥尾逗号补句号、后随逗号改句号分隔)/ 兜底旧前缀拼接;三分支各打一条 console log 便于实跑核对落点
