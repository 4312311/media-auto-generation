...
| 自动回复 | `autoreply` | 挂机:每轮 AI 回复结束(`GENERATION_ENDED`)延迟 N 秒自动把固定内容作为用户消息发出(`sendTextareaMessage` 原生路径,`MESSAGE_SENT` 确认落库才消耗计数,拦截态不消耗并还原输入框),发满自动关。内容支持 `{n}`/`{}` 占位符替换为当前第几轮。流式卡住看门狗(超阈值无新内容 → stopGeneration 掐断 → `#option_regenerate` 重生成,重试耗尽自动停用)。**上一楼 echo 校验**(2026-08-30 加,`fireAutoReply` 发送前):最后一楼仍是"自动回复产物"(AI 空回复/楼层回滚/手动删 AI 楼导致上轮没人接)时不再堆叠新自动回复,改为 trigger `#option_regenerate` 让 AI 回复那条(ST 对最后一楼是 user 的 regenerate 即生成 AI 回复,script.js:11567);判定走 `isAutoReplyEcho` 模板挖空正则(模板按 `{n}`/`{}` 切段转义、段间接 `\d+`,未配占位符退化为全等);重试不发新内容不消耗计数(下次真发时轮次数字自然接续);连续未获回复超过上限(复用「卡住重试次数」配置)自动停用,正常发出一条后 `autoReplyFloorRetries` 清零。**剩余次数可直接编辑**(`#auto_reply_remaining` 数字输入框,2026-08 加,用户误清零/耗尽自动关后手动补次数的通道;写入钳制 0..maxCount,启用状态下补次数且空闲时会立即续跑)。**启用时余额 >0 保留、为 0 才重置满额**(旧版"启用即重置满额"会覆盖手动设置的余额,正是"次数回不去"的根因);『重置次数』按钮仍一键回满额。停止双保险见事件监听一节 |
...
**媒体预览浮窗**(流式看图用):发送栏图标 `#mag_preview_btn`(`#rightSendForm` 内,fa-images)打开居中 modal `#mag_media_preview_modal`,按楼层(新→旧)列出当前聊天全部生成媒体(含流式未落地 in-flight),层内按创建时间正序,一行一个可滚动;层间分隔线 + 『最新 / 第 N 楼的图片』标签;行内布局(2026-09-13 起):最上是元信息行 `.preview-media-time`(`第 N 张` 序号 + 创建时间 + 前缀名称徽标 + 行尾复制按钮),其下 declare 描述(`PIC_DECLARE_RE` 兼容 `[img_Declare]...[/img_Declare]` / `[video_Declare]...[/video_Declare]`(2026-08 起用户卡的 BBCode 新格式,图片/视频各自带 declare)与 `<pic_Declare>...</pic_Declare>`(旧卡)三种,开/闭标签交替保持单捕获组;**用户角色卡的 AI 输出惯例**:declare 块包着图片描述、紧贴 [image]/[video] 标签,中间只隔空白/换行;隔着其他内容则不关联;declare 是用户卡的约定,插件只读不约束格式),最下才是媒体本体;元信息行字段:创建时间(优先 `galleryManifest.timestamp`=生成完成时刻,回退 magId 内嵌 base36 时间戳)、**前缀名称徽标** `.preview-media-prefix`(生成时用的前缀条目名,`pickPositivePrefix` 返回的 name 快照经 buildMediaWrap bake 进 wrapper `data-prefix-name`、manifest 条目 `prefixName` 双路持久;存量按 url 回查 manifest,查不到不显示),行尾复制按钮 `.preview-media-copy-btn`(委托点击,`copyText` 优先降级原生剪贴板,**复制该行 `data-final-prompt`=生成时刻拼装的完整 prompt 快照**,空则退裸 `data-prompt`——存量媒体按 url 回查 manifest 补,再没有就裸 prompt;点媒体放大的 lightbox 同样传 finalPrompt,与图库路径一致。快照在 `buildMediaWrap` 生成时 bake 进 wrapper 的 `data-final-prompt` 属性,跟 mes 持久走,切配置/改前缀后复制到的仍是当时生成用的完整 prompt,不动态重拼;manifest 的 `prompt` 字段六个写入点本来就存 finalPrompt 快照,图库 lightbox 复制天然是它)。数据源独立于消息 DOM:`collectFloorMedia()` 扫各楼层 `message.mes` 的 mag-media wrapper(`MAG_MEDIA_WRAP_RE`)+ 旧格式裸 `<img|video prompt=...>`(先剥 wrapper 再扫,避免空扫 MB 级 base64)+ 流式 `inFlightMedia`。自动刷新:`scheduleMediaPreviewRender()` 挂在 pushGalleryEntry / commitMediaToMessage / processMessageContent 落地 / GENERATION_STARTED / MESSAGE_EDITED / CHAT_CHANGED。**「只看未生成/失败」过滤**(header checkbox,会话级不持久化,右侧计数徽标=可重试条数):`collectUnfulfilledMedia()` 收集两类未落地条目——① mag-placeholder 占位符(`MAG_PLACEHOLDER_RE`,state=error→失败带原因/idle→未生成/loading→生成中,magId 首段即 promptHash);② 休眠裸 `[image]/[video]` 标签(processMessageContent 只扫最后一楼,新楼层开始后旧楼标签永不触发——正是本功能要救的场景;剥 wrapper/占位符后 image/video 两套正则都扫,与 mediaType 设置无关;hash 分类与逻辑 B 同算法:processingHashes/inFlightMedia 命中=生成中、failedPrompts 命中=失败、其余=未生成;流式目标楼跳过,半截标签不判定)。条目行含状态徽章/prompt/失败原因/重试按钮,点重试走 `retryUnfulfilledMedia()`:占位符按 magId→commitMediaToMessage、裸标签按 floor+originalTag→commitBareTagToMessage 原位替换(都纯 mes 数据层操作,不依赖该楼 DOM 渲染);显式用户操作绕过冷却;失败写 failedPrompts(行即时变失败态);落点丢失(楼层被编辑)时媒体仍入图库保底。
...
- `collectFloorMedia()` / `collectUnfulfilledMedia()` / `declareForPosition()` / `renderMediaPreviewModal()` / `buildPendingRowHtml()` / `scheduleMediaPreviewRender()` — 媒体预览浮窗(含「只看未生成/失败」过滤,见功能概览)
- `commitMediaToMessage(magId, mediaWrap, caller)` — 手动/重生成按 magId 反查任意楼层落地;`commitBareTagToMessage(floor, originalTag, mediaWrap, caller)` — 未生成重试的裸标签按原文原位落地;`retryUnfulfilledMedia(rec)` — 预览浮窗「未生成/失败」重试入口(见功能概览)
...
- `applyStylePrefixToPrompt` — 正向风格前缀拼接(`generateViaComfyInner` 内唯一拼装点,自动/手动/重生成/测试/重试全路径共用):`[%style%]` 占位符原位替换(split/join 字面全量替换,无 `$` 展开语义)/ `[Shot 1]` 后插入(前缀剥尾逗号补句号、后随逗号改句号分隔)/ 兜底旧前缀拼接;三分支各打一条 console log 便于实跑核对落点。拼装结果即 `finalPrompt`,五个 wrapper 构造点把它 bake 进 `data-final-prompt`(见 buildMediaWrap)
- `prefixEntryCount` / `clampPrefixIndex` / `ensurePrefixList` / `migrateLegacyPrefixes` / `pickPositivePrefix` / `renderPrefixList` — 多前缀 + 循环轮换系统(见功能概览 ComfyUI tab 行):次数/选中行钳制读取的两个共享助手;数组守卫;loadSettings 一次性旧字段迁移;生成时取前缀(循环游标推进+持久化,console log 带 row 序号);列表 UI 渲染(行内元素启停归它所有——radio 置灰由循环开关决定,**不能**进 renderPresetFields 那条统一 `prop('disabled', !hasPreset)` 行,统一 enable 会洗掉循环置灰态,E2E 实测过的坑)。duplicatePreset 深拷贝列表+游标归零(浅拷贝会两档共享数组互相污染);fetchAndApplyImportUrl 前缀整体换血为单条列表
- `buildMediaWrap` — mag-media wrapper 构造(占位符替换/自动/手动/重生成/重试/楼层重生共用):`rawPrompt` 进 `data-prompt`(裸 prompt,ST Regex 还原规则锚点),`finalPrompt`(生成时刻完整拼装,含前缀)可选 bake 进 `data-final-prompt`,`prefixName`(同次生成用的前缀条目名)可选 bake 进 `data-prefix-name`(为空整个属性缺席);`parseMediaWrapper` 对称解析回字段
...
- `videoShouldMute` / `applyVideoMuted` / `playFloorVideoOnScroll` / `attachFloorVideoControl` / `scanFloorVideos` / `resolveFloorVideoPlayback` / `initFloorVideoPlaybackControl` — 楼层视频播放控制(滑入视口自动播放 + 离屏自动暂停 + 同屏单播 + 默认声音开关,2026-09 起由 7e4e3f0 的"禁自动播放点播制"改为短视频式滑到即播):IntersectionObserver **只记账不播**——回调仅刷新在屏名单 `floorVideoVisibility`(Map,el→是否在视口内,断链即删防强引用泄漏),播谁由 `resolveFloorVideoPlayback` 统一裁决;**同屏单播**(2026-09-19 加,原实现同屏两条各播各的):同屏可见的楼层视频至多播一个,winner 优先维持 `activeFloorVideo` 当前活跃(滚动中新视频露头不抢播)、不在/被掐才轮到 DOM 顺序最上,winner 被用户手动暂停过则顺延下一个未被手动暂停的(全部在屏都被手动暂停→谁也不播),其余在屏的一律掐停(`floorVideoIoPaused` 标记,不算用户暂停,之后轮到时照常自动播);离屏一律掐停(裁决函数里离屏名单兜底,threshold 0);**用户手动点播可抢占**(attach 的 play 监听,`floorVideoPolicyPlay` WeakSet 标记区分策略发起/用户点播):点播的立即成为唯一在播、原在播条让位掐停;用户手动 pause 后**不**立即让别的视频顶上(等下次 IO 事件即滚动才重算);**手动暂停记忆**(2026-09-10 加)——用户经控件手动 pause 过的视频滑回视口不硬拉重播、手动点播放后恢复自动播,靠 `floorVideoUserPaused`/`floorVideoIoPaused` 两个 WeakSet 区分:pause 事件异步派发,程序化(离屏/摘 autoplay)pause 前先标记、监听里消费即删,剩下的才算用户暂停;自动播放与声音偏好解耦——`videoDefaultSound` 开(带声)时手机浏览器拦截无手势的有声自动播放,catch 后降级静音照播(想听声点 controls 开),"滑到即播"恒成立。wrapper HTML 恒 baked `muted` 作安全初值(换 src 重载会按 attribute 重置 muted 态,所以 `applyVideoMuted` property+attribute 双写),实际行为由 attach 时按 `videoDefaultSound` 覆写 property 决定——存量楼层免改 mes,数据层与设备偏好解耦;attach 有 WeakSet 挡重复,开关切换时由 checkbox handler 对已渲染楼层视频批量重刷;图库 lightbox 与测试预览在换 src 前先行应用偏好(lightbox 由用户点击打开带手势,非静音 autoplay 浏览器放行)
...
8. 楼层视频滑入视口自动播放 → 滚到视频楼层即播、滚走暂停、滚回来续播(loop);**同屏两条只播一个**(2026-09-19 加):首屏同时见到两条只有 DOM 靠上的播、另一条待命,在播的滚走后待命的自动接上;手动点播待命条会抢占、原在播条让位;声音开关开(带声)时手机上被浏览器拦截会自动降级静音照播(滑到必播,不受声音偏好拖累);手动暂停过的滚走再滚回**不**重播,手动点播放后恢复自动播
9. 自动回复 echo 校验 → 删掉 AI 末楼(使最后一楼变成上轮自动回复)后等延迟到期:应触发 regenerate 重试而非再发一条;重试后 AI 正常回复则链路恢复照常发下一条;AI 连续不回(可临时把回复内容改成必触发空回的形式)超上限应自动停用
10. 风格前缀 `[Shot 1]` 格式感知 → 三分支:prompt 带 `[%style%]`(如 `[Shot 1][%style%], light`)→ 原位换;带 `[Shot 1]` 无占位符(如 `[Shot 1], light. Lisa`)→ 前缀插标记后、句号结尾、原逗号删掉;无标记(普通图片 prompt)→ 旧行为前缀拼最前。console 有 `style prefix: ...` log 可核对
11. 复制生成时完整 prompt → 媒体预览行尾复制/预览放大 lightbox/图库 lightbox 复制到的都应是**生成时刻**的完整 prompt(含当时前缀);生成后改前缀或切配置档再复制,内容不变(不动态重拼)。存量媒体(无 `data-final-prompt`)按 url 回查 manifest 兜底,manifest 也删了就退裸 prompt;wrapper 内联复制按钮(聊天楼层里)仍是裸 `data-prompt`,刻意不动
12. 多前缀循环 → 配 A(1)/B(2)/C(2) 开循环连发:console `style prefix cycle: 使用「名称」(N/M)` 与复制到的 finalPrompt 前缀应按 A,B,B,C,C,A 轮换;某条次数设 0 → 轮换跳过它(A1/B0/C2 → A,C,C 循环);全表 0 → 生成不带前缀;刷新页面后接着上次进度走;关循环恒用 ⊙ 选中行(该行次数 0 也照常使用);改小 count/删行后下一次取值按快进/取模自愈;复制配置档新档从第一条走、两档列表互不影响;旧单值配置档刷新后自动迁移为列表第一项(全档一次性);每行名称输入框改完落盘,刷新后保留(存量条目名称为空正常回退)
