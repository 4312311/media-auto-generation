...
- `buildMediaWrap` — mag-media wrapper 构造(占位符替换/自动/手动/重生成/重试/楼层重生共用):`rawPrompt` 进 `data-prompt`(裸 prompt,ST Regex 还原规则锚点),`finalPrompt`(生成时刻完整拼装,含前缀)可选 bake 进 `data-final-prompt`(为空整个属性缺席);`parseMediaWrapper` 对称解析回两个字段
