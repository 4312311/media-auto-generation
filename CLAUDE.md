...
8. 楼层视频滑入视口自动播放 → 滚到视频楼层即播、滚走暂停、滚回来续播(loop);声音开关开(带声)时手机上被浏览器拦截会自动降级静音照播(滑到必播,不受声音偏好拖累);手动暂停过的滚走再滚回**不**重播,手动点播放后恢复自动播
