...
**生成超时与手动中断**
- 生成超时：视频 5 分钟、图片 1 分钟（视频多帧采样耗时数分钟，超时前台会报错终止）。
- 手动模式下可以中途终止生成：点击正在转圈的占位符会弹出确认框，点「终止生成」即中断——前端请求取消 + ComfyUI 端正在执行的任务同步打断（要求 ComfyUI 启动带 `--enable-cors-header`）。终止后占位符恢复为可点击的「生成图片/视频」状态，随时可重新生成。
...
- 提示词里的撇号（`'`）会以原样回传给 LLM（插件存储时不再把它转义成 `&#39;`，历史消息里的旧实体也会在进入聊天时自动压回撇号）；`"`/`&`/`<`/`>` 这四个字符在 HTML 属性里必须转义（`&quot;` 等），还原后发给 LLM 时会以实体形式出现，属已知边界，一般不影响生成。
